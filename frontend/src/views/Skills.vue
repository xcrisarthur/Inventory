<template>
    <div>
        ...
        <table class="table table-striped text-center">
            ...
            <tbody>
                <tr v-for="skill in filteredSkills" :key="skill.id_skill">
                    <td>{{ skill.id_skill }}</td>
                    <td>{{ skill.nama }}</td>
                    <td>{{ skill.level }}</td>
                    <td>{{ skill.notes }}</td>
                    <td>
                        <button type="button" class="btn btn-warning"
                            @click="redirectToEditPage(skill.id_skill)">Edit</button>
                        <button type="button" class="btn btn-danger ms-2"
                            @click="deleteItem(skill.id_skill)">Delete</button>
                    </td>
                </tr>
            </tbody>
        </table>
    ...
    computed: {
        ...
        filteredSkills() {
            return this.skills.filter(skills =>
                this.searchSkills(skills, this.searchText.toLowerCase()) &&
                (this.selectedNama === 'Nama' || skills.nama === this.selectedNama) &&
                (this.selectedLevel === 'Level' || skills.level === this.selectedLevel)
            );
        },
        filteredSkillsCsv() {
            return this.skillsCsv.filter(skills =>
                this.searchSkills(skills, this.searchText.toLowerCase())
            );
        },
        uniqueNamas() {
            const namas = this.skills.map(skills => skills.nama).filter(Boolean);
            return [...new Set(namas)];
        },
        uniqueLevels() {
            const levels = this.skills.map(skills => skills.level).filter(Boolean);
    ...
    methods: {
        ...
        deleteItem(itemId) {
            if (confirm('Are you sure you want to delete this item?')) {
                axios.delete(`http://localhost:8080/api/skills/${itemId}`)
                    .then(() => {
        ...
        redirectToAddPage() {
            window.location.href = 'http://localhost:8081/SkillsADD';
        },
        redirectToEditPage(itemId) {
            router.push({ path: `/Skills/${itemId}` });
        },
        generatePDFPreview() {
            const doc = new jsPDF();
