    ...
    login() {
      ...
      .then(data => {
        ...
        if (data.message === "login berhasil") {
          ...
          this.setCookie("token", data.token, 1); // Expires in 1 day

          // Redirect to a different page or update state
          console.log(this.setCookie("token", data.token, 1), "  ", data.token)

          this.$router.push({ path: '/dashboard' }); // Replace with your desired path
        } else {
