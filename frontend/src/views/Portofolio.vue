<template>
    <div>
        ...
        <table class="table table-striped text-center">
            ...
            <tbody>
                <tr v-for="portofolio in filteredPortofolios" :key="portofolio.id_portfolio">
                    <!-- <td>{{ portofolio.id_portfolio }}</td> -->
                    <td>{{ portofolio.nama }}</td>
                    <td>{{ portofolio.tanggal }}</td>
                    <td>{{ portofolio.deskripsi }}</td>
                    ...
                    <td>
                        <button type="button" class="btn btn-warning"
                            @click="redirectToEditPage(portofolio.id_portfolio)">Edit</button>
                        <button type="button" class="btn btn-danger ms-2"
                            @click="deleteItem(portofolio.id_portfolio)">Delete</button>
                    </td>
                </tr>
            </tbody>
        </table>
...
export default {
    ...
    computed: {
        ...
        filteredPortofolios() {
            return this.portofolios.filter(portofolios =>
                this.searchPortofolios(portofolios, this.searchText.toLowerCase()) &&
                (this.selectedRole === 'Role' || portofolios.role === this.selectedRole) &&
                (this.selectedTechnology === 'Technology' || portofolios.technology === this.selectedTechnology)
            );
        },
        filteredPortofoliosCsv() {
            return this.portofoliosCsv.filter(portofolios =>
                this.searchPortofolios(portofolios, this.searchText.toLowerCase())
            );
        },
        uniqueRoles() {
            const roles = this.portofolios.map(portofolios => portofolios.role).filter(Boolean);
            return [...new Set(roles)];
        },
        uniqueTechnologys() {
            const technologys = this.portofolios.map(portofolios => portofolios.technology).filter(Boolean);
    ...
    methods: {
        ...
        deleteItem(itemId) {
            if (confirm('Are you sure you want to delete this item?')) {
                axios.delete(`http://localhost:8080/api/portfolio/${itemId}`)
                    .then(() => {
        ...
        redirectToAddPage() {
            window.location.href = 'http://localhost:8081/PortofolioADD';
        },
        redirectToEditPage(itemId) {
            router.push({ path: `/Portofolio/${itemId}` });
        },
        generatePDFPreview() {
            const doc = new jsPDF();
